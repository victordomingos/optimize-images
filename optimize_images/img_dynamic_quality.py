...
def get_diff_at_quality_pil(photo, quality):
    """Return a difference score for this JPEG image saved at the specified quality
...
def get_diff_at_quality_np(photo, quality):
    """Return a difference score for this JPEG image saved at the specified quality
...
def jpeg_dynamic_quality_pil(original_photo, use_dynamic_quality=True):
    """Return an integer representing the quality that this JPEG image should be
    saved at to attain the quality threshold specified for this photo class.
    ...
    if not use_dynamic_quality:
        default_diff = get_diff_at_quality_pil(photo, hi)
        return hi, default_diff

    # 95 is the highest useful value for JPEG. Higher values cause different behavior
    # Used to establish the image's intrinsic ssim without encoder artifacts
    normalized_diff = get_diff_at_quality_pil(photo, 95)
    ...
    for i in range(_diff_iteration_count(lo, hi)):
        curr_quality = (lo + hi) // 2
        curr_diff = get_diff_at_quality_pil(photo, curr_quality)
        diff_ratio = curr_diff / normalized_diff
...
def jpeg_dynamic_quality_np(original_photo, use_dynamic_quality=True):
    """Return an integer representing the quality that this JPEG image should be
    saved at to attain the quality threshold specified for this photo class.
    ...
    if not use_dynamic_quality:
        default_diff = get_diff_at_quality_np(photo, hi)
        return hi, default_diff

    # 95 is the highest useful value for JPEG. Higher values cause different behavior
    # Used to establish the image's intrinsic ssim without encoder artifacts
    normalized_diff = get_diff_at_quality_np(photo, 95)
    ...
    for i in range(_diff_iteration_count(lo, hi)):
        curr_quality = (lo + hi) // 2
        curr_diff = get_diff_at_quality_np(photo, curr_quality)
        diff_ratio = curr_diff / normalized_diff
